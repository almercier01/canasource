/*
  ...
  1. Changes
    - Create business_images table if not exists
    - Add indexes for performance
    - Create secure view for business details
    - Set up RLS policies
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'business_images') THEN
    ...
    CREATE INDEX IF NOT EXISTS idx_business_images_status ON business_images(status);
    CREATE INDEX IF NOT EXISTS idx_business_images_created_at ON business_images(created_at DESC);
  END IF;
END $$;
